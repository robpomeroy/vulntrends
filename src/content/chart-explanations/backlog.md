...
A growing backlog for a manufacturer over multiple consecutive years can
indicate that patching throughput is not keeping pace with new disclosures (or
that patch-date coverage is incomplete). Compare with the "discovered" chart and
the patch-date coverage limitations above.
